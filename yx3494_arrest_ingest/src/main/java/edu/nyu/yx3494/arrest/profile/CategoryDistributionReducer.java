...
public class CategoryDistributionReducer extends Reducer<Text, IntWritable, Text, Text> {
    Map<String, Integer> categoryDistribution = new HashMap<>();
    Pair<String, Integer> max = new Pair<>("", 0);
    Pair<String, Integer> min = new Pair<>("", Integer.MAX_VALUE);

    // Static the category counter
    ...
    protected void cleanup(Reducer<Text, IntWritable, Text, Text>.Context context) throws IOException, InterruptedException {
        context.write(new Text("Total Categories"), new Text(String.valueOf(categoryDistribution.size())));
        context.write(new Text("Most Frequent Category"), new Text(max.getKey() + " (" + max.getValue() + " records)"));
        context.write(new Text("Least Frequent Category"), new Text(min.getKey() + " (" + min.getValue() + " records)"));
        context.write(new Text("================== All Category =================="), new Text(""));
